...
describe('Basic wallet navigation', () => {
  beforeEach(async () => {
  ...
  */
  ...
  it('should see Profile screen after swiping right', async () => {
    await element(by.id('EmptyAssetList')).swipe('right', 'fast', 0.5);
    await expect(element(by.id('ProfileMasthead'))).toBeVisible();
  });

  it('should see Profile screen after tapping Profile header button', async () => {
    await element(by.id('goToProfile')).tap();
    await expect(element(by.id('ProfileMasthead'))).toBeVisible();
  });
});
